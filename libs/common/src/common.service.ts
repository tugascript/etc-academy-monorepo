...
import { IAccessPayloadResponse, IAccessUser } from 'app/common/interfaces';
import { join } from 'path';
import { readFileSync } from 'fs';
import { verifyToken } from 'app/common/utils';
...
export class CommonService {
  ...
  public async verifyAccessToken(token: string): Promise<IAccessUser> {
    try {
      const payload: IAccessPayloadResponse = await verifyToken(
        token,
        publicKey,
      );
